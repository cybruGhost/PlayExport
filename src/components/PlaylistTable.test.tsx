...
test("redirecting when access token is invalid", async () => {
  // @ts-ignore
  window.location = { href: "http://www.example.com/playExport#access_token=INVALID_ACCESS_TOKEN" }

  render(<PlaylistTable accessToken="INVALID_ACCESS_TOKEN" onSetSubtitle={onSetSubtitle} />)

  await waitFor(() => {
    expect(window.location.href).toBe("http://www.example.com/playExport")
  })
})
